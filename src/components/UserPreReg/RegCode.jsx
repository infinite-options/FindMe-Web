    ...
    const verifyRegCode = () => {
        axios
          .get(BASE_URL + `verifyRegCode/${regCode}`)
            .then((response) => {
              if (response.data.result.result.length !== 0 && response.data.result.result[0] !== undefined) {
                setEvent(response.data.result.result[0]);
                console.log(response.data.result.result[0]);
                navigate('/eventProfile', { state: { event: response.data.result.result[0] } })
              }
              else {
                setShowError(true);
    ...
    return (
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', paddingTop: "5%" }}>
            ...
            >
                ...
                <Button variant="outlined" sx={{mt: 2}} onClick={() => verifyRegCode()}> Verify </Button>
                { showError && <Typography sx={{mt: 2}}>Invalid Code. Please try again.</Typography>}
            </Grid>
        </div>
        </>
    )
